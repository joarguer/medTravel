...
ALTER TABLE `travel_packages` 
  ADD COLUMN IF NOT EXISTS `medtravel_fee_type` ENUM('fixed','percent') DEFAULT 'percent' COMMENT 'Tipo de tarifa MedTravel',
  ADD COLUMN IF NOT EXISTS `medtravel_fee_value` DECIMAL(10,2) DEFAULT 0.00 COMMENT 'Valor de tarifa: $ fijo o % porcentaje',
  ADD COLUMN IF NOT EXISTS `medtravel_fee_amount` DECIMAL(10,2) DEFAULT 0.00 COMMENT 'Monto calculado de tarifa MedTravel',
  ADD COLUMN IF NOT EXISTS `provider_commission_value` DECIMAL(10,2) DEFAULT 0.00 COMMENT 'Comisión pagada al proveedor',
  ADD COLUMN IF NOT EXISTS `gross_margin` DECIMAL(10,2) DEFAULT 0.00 COMMENT 'Margen bruto (total - costos)',
  ADD COLUMN IF NOT EXISTS `net_margin` DECIMAL(10,2) DEFAULT 0.00 COMMENT 'Margen neto (después de comisiones)';
...
-- Índices para reportes comerciales
ALTER TABLE `travel_packages` ADD INDEX IF NOT EXISTS `idx_margins` (`status`, `net_margin`);
ALTER TABLE `travel_packages` ADD INDEX IF NOT EXISTS `idx_payment_status_dates` (`payment_status`, `start_date`);
