...
CREATE TABLE IF NOT EXISTS `blog_posts` (
  ...
  `author_user_id` INT NULL DEFAULT NULL COMMENT 'FK to usuarios.id when the post is authored by a specific platform user',
  ...
  KEY `idx_blog_status_published_at` (`status`, `published_at`),
  KEY `idx_blog_provider` (`provider_id`),
  KEY `idx_blog_author_user` (`author_user_id`)
