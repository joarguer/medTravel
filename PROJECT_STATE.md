...
- **Fecha última actualización de este archivo:** 2026-04-20 (cierre de sesión — prefill desde ConectarBot/Chatwoot en booking asistido)
...
- **2026-04-20** — feat(booking): `admin/booking_asistido.php` acepta prefill GET desde ConectarBot/Chatwoot (`prefill_email`, `prefill_name`, `prefill_phone`, `prefill_channel`, `cw_conversation_id`, `cw_contact_id`). Banner de confirmación chatwoot. Auto-lookup de email al cargar. `admin/ajax/booking_asistido.php` persiste `cw_conversation_id` en `booking_requests`. Migración: `sql/2026_04_20_booking_cw_conversation.sql`. El booking sigue siendo 100% agent-assisted; el label en chatwoot solo dispara la URL prefill, no crea bookings automáticamente.
