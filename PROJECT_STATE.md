...
- **Fecha última actualización de este archivo:** 2026-08-20 (`GET /api/conectarbot/v1/catalog/offer/{id}` implementado y endurecido tras NO-GO de revisión: redacción de contacto en textos libres y filtrado staff↔PCS exacto)
...
- **2026-08-20** — fix(conectarbot/offer): corregidos 2 hallazgos de revisión (NO-GO) sobre `GET /api/conectarbot/v1/catalog/offer/{id}` en `api/conectarbot/v1/index.php`. (1) Privacidad en texto libre: `offer.description`, `service.description` y `staff[].description` ahora pasan por `cbot_redact_contact_info()`, wrapper nuevo que reutiliza el redactor canónico ya existente en el repo `commission_gate_redact_sensitive()` (`inc/commission_gate.php`, usado hoy para ocultar contacto en payloads de inbox/booking pre-pago) para enmascarar emails/teléfonos/handles de WhatsApp-Telegram-etc. antes de responder; contenido médico/comercial normal se preserva intacto. Se agregó `require_once inc/commission_gate.php` (archivo sin efectos de carga, solo definiciones de función guardadas con `function_exists`). (2) Staff↔PCS: `cbot_fetch_service_staff()` gana parámetros opcionales `?int $providerId = null, ?int $pcsId = null` (retrocompatibles; `service_detail()` sigue llamándola sin ellos, sin cambio de comportamiento) que filtran a nivel SQL por `pms.provider_id` y por `rel.provider_catalog_service_id` cuando la oferta trae PCS y la columna existe en el esquema; si la oferta no trae PCS o la relación `provider_medical_staff_services` no modela esa columna, se conserva el fallback real de provider+service sin inventar asociación. Antes solo se filtraba por `provider_id` en PHP sobre un array ya deduplicado, lo que podía aceptar staff de un PCS distinto. Validado localmente contra `medtravel_rebuild_20260415` (MAMP MySQL 8889 + PHP built-in server, sesión de solo lectura): oferta `id=1` (provider=1, service=8, pcs=1) sigue devolviendo 200 con staff/locations correctos; probe SQL directo confirma que `pcs=1` matchea la relación real, `pcs=999` (simulando PCS distinto) la excluye, y `pcs=null` (offer sin PCS) usa el fallback provider+service; `commission_gate_redact_sensitive()` probado directo con texto con email → `[email redacted]`, texto con teléfono → `[phone redacted]` (con un artefacto cosmético preexistente del regex de teléfono al dejar un paréntesis suelto en un caso, no introducido por este cambio y fuera del alcance de archivos autorizados), texto normal → intacto. `php -l` y `git diff --check` limpios. Sin escrituras a BD, sin tocar ConectarBot, sin commit/push/deploy.
- **2026-08-20** — feat(conectarbot): implementado `GET /api/conectarbot/v1/catalog/offer/{id}` en `api/conectarbot/v1/index.php` sobre el canon documental ya existente. Ruta acepta solo `{id}` numérico; formato inválido cae en `404 NOT_FOUND` genérico de ruta. Handler `offer_detail()` valida en cascada: oferta activa/no eliminada, `service_catalog` activo/no eliminado, `providers` activo/no eliminado (todo vía INNER JOIN + helpers `cbot_table_has_column`/`cbot_table_exists` ya usados en el resto del archivo), y coherencia de `provider_catalog_service_id` cuando la oferta lo trae (`cbot_offer_pcs_valid()`: mismo provider/service y `is_active=1` si la columna existe). Cualquier fallo de estas validaciones responde `404 NOT_FOUND` para fallback silencioso de ConectarBot. Staff reutiliza `cbot_fetch_service_staff()` (ya exigía `provider_medical_staff_services.active`, `provider_medical_staff.is_active`/`active` y `allow_home_publication=1`) filtrado por `provider_id` de la oferta; soporta múltiples staff sin colapsar. `locations` reutiliza `cbot_locations_from_catalog()`. Response sin `phone`/`email`/`whatsapp`/`mobile`/`cell`/`address`; `clinic_name` conserva semántica de texto comercial del staff. Validado localmente contra `medtravel_rebuild_20260415` (MAMP MySQL puerto 8889, arrancado en la sesión con `startMysql.sh`) usando PHP built-in server: oferta válida `id=1` (provider_id=1, service_id=8, con `provider_catalog_service_id=1` coherente) devuelve 200 con staff/locations correctos y sin datos privados; `id=999999` devuelve 404 `NOT_FOUND`; `id` no numérico cae en 404 genérico de ruta. El dataset local solo tiene 1 oferta activa, por lo que los casos "oferta inactiva", "provider/service inactivo", "staff múltiple" y "ausencia de staff" quedaron validados por revisión de código (mismos helpers ya probados en `service_detail`) y no por request en vivo; branching de columnas opcionales (`is_deleted`, `is_active`/`active`, `allow_home_publication`, `provider_catalog_service_id`) confirmado contra `information_schema.columns` real. `php -l` y `git diff --check` limpios. Sin escrituras a BD, sin commit/push/deploy. Endpoints existentes intactos.
- **2026-08-20** — docs(conectarbot/meta): se canoniza el contrato futuro `GET /api/conectarbot/v1/catalog/offer/{id}` para que ConectarBot pueda enriquecer conversaciones de WhatsApp originadas en campañas Meta cuando reciba `offer_id` desde el canal. Sin `offer_id`, el flujo sigue siendo genérico. Con `offer_id` válido y oferta/servicio/provider activos, MedTravel deberá devolver contexto público de la oferta; con ID inválido, inactivo o no publicable, responderá `404 NOT_FOUND` para fallback silencioso al flujo genérico. La respuesta pública permite oferta, precio, servicio, provider, staff publicable y ubicaciones seguras; prohíbe contactos directos (`phone`, `email`, WhatsApp, mobile/cell), dirección exacta y datos equivalentes. `clinic_name` conserva semántica de texto comercial del staff, no entidad clínica formal. Este canon quedó implementado el mismo día — ver entrada superior `feat(conectarbot)` 2026-08-20 para el estado real vigente.
