...
if (empty($scope['ok'])) {
    admin_notif_err((string)($scope['message'] ?? 'forbidden'), (int)($scope['status'] ?? 403));
}

$isAdmin = !empty($scope['is_admin']);
$providerId = (int)($scope['provider_id'] ?? 0);
$serviceProviderId = (int)($scope['service_provider_id'] ?? 0);
$readerRole = (string)($scope['reader_role'] ?? 'PROVIDER');
$userId = (int)($scope['user_id'] ?? 0);
$scopeWhere = (string)($scope['scope_where'] ?? '');
$scopeTypes = (string)($scope['scope_types'] ?? '');
$scopeParams = (array)($scope['scope_params'] ?? []);
...
if ($isProviderScope && inbox_table_exists($conexion, 'booking_request_items') && inbox_table_exists($conexion, 'booking_requests')) {
    ...
    $pendingStatusExpr = $hasItemStatus
        ? "CASE
