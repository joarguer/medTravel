...
if ($tipo == 'get_header') {
    ...
    if (mysqli_num_rows($table_check) == 0) {
        echo json_encode([
            ...
            'message' => 'La tabla offer_detail_header no existe. Por favor ejecute el script SQL de instalación.'
        ]);
        exit;
    }
    
    $query = mysqli_query($conexion, "SELECT * FROM offer_detail_header WHERE activo = '0' ORDER BY id ASC LIMIT 1");
    ...
    if (!$header) {
        ...
        $insert = mysqli_query($conexion, "INSERT INTO offer_detail_header (title, subtitle_1, subtitle_2, activo) VALUES ('Medical Service Details', 'OFFER DETAILS', 'Complete information about your medical service', 0)");
        if ($insert) {
            $query = mysqli_query($conexion, "SELECT * FROM offer_detail_header WHERE activo = '0' ORDER BY id ASC LIMIT 1");
            $header = mysqli_fetch_assoc($query);
        }
...
if ($tipo == 'create_header') {
    $title = isset($_POST['title']) ? $_POST['title'] : 'Medical Service Details';
    $subtitle_1 = isset($_POST['subtitle_1']) ? $_POST['subtitle_1'] : 'OFFER DETAILS';
    $subtitle_2 = isset($_POST['subtitle_2']) ? $_POST['subtitle_2'] : 'Complete information about your medical service';
    
    $stmt = mysqli_prepare($conexion, "INSERT INTO offer_detail_header (title, subtitle_1, subtitle_2, activo) VALUES (?, ?, ?, 0)");
...
if ($tipo == 'edit_campo') {
    ...
    $stmt = mysqli_prepare($conexion, "UPDATE offer_detail_header SET $campo = ? WHERE id = ?");
...
if ($tipo == 'upload_image') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        $new_filename = 'offer-detail-header-' . time() . '.' . $file_extension;
        $upload_path = $upload_dir . $new_filename;
        
        if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)) {
            $bg_image = 'img/site/' . $new_filename;
            
            $stmt = mysqli_prepare($conexion, "UPDATE offer_detail_header SET bg_image = ? WHERE id = ?");
